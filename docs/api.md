...
| Automations                                | `GET/POST …/automation/rules`, `GET/PUT/PATCH/DELETE …/automation/rules/:id`, `GET …/automation/rules/:id/executions`                                                                                                          |
| Knowledge                                  | `GET/POST …/knowledge`, `GET/PATCH/DELETE …/knowledge/:id`, `GET …/knowledge/search?q=&limit=`, `GET …/knowledge/for-incident/:incidentId`                                                                                     |
...
| AI investigation                           | `GET …/ai/status`, `GET/POST …/incidents/:incidentId/investigations`                                                                                                                                                           |
| Realtime                                   | `GET /orgs/:orgId/events` (SSE; see below)                                                                                                                                                                                     |
| Ops                                        | `GET /health/live`, `GET /health/ready`                                                                                                                                                                                        |

### Knowledge base

`GET …/knowledge` lists document summaries (optional `q` filters by title, `tag` by tag), each with `index: { chunks, embedded }` so a client can show that meaning-based search is still catching up. `POST` and `PATCH` take `{ title, contentMd, tags }` (at most 200 characters, 100 000 characters and 10 tags; a document body is Markdown and is stored and returned verbatim, never interpreted by the API). `GET …/knowledge/search?q=` returns `{ mode, data }`; `mode` is `hybrid`, or `keyword` when meaning-based search was unavailable. Each hit has `documentId`, `title`, `heading` (where it matched), a plain-text `snippet`, a 0 to 1 `score` and `matchedBy` (`keyword`, `semantic` or both). `GET …/knowledge/for-incident/:id` returns the same shape for up to five runbooks relevant to an incident. Reading needs `knowledge.read` (every role); writing needs `knowledge.manage` (developer, admin, owner). Errors: 400 (validation), 404 (unknown, malformed or foreign id), 409 `DOCUMENT_LIMIT` (500 per organization), 429 (60 searches per minute per user).

### Account security

`POST /auth/logout-all` (204) revokes every session of the current user on every device and clears the cookie. `POST /auth/change-password` `{ currentPassword, newPassword }` (204) needs the current password (400 `INVALID_CURRENT_PASSWORD` if wrong, deliberately not 401), applies the password policy, keeps the current session and ends every other one. `POST /auth/forgot-password` `{ email }` always answers **202 `{ accepted: true }`** whether or not the address has an account, and emails a link to the account's own address; limited to 3 per address per hour. `POST /auth/reset-password` `{ token, password }` (204) sets the password, ends every session and uses the token up; an unknown, used or expired token is 400 `INVALID_RESET_TOKEN`. Tokens are single-use and expire after an hour. All of these are audited.

`GET /metrics` (Prometheus text) does not exist (404) unless `METRICS_TOKEN` is set, then needs `Authorization: Bearer <token>`. Every response carries `X-Request-Id`; a client that reports a problem should quote it. The whole API is limited per client IP (default 1 200 a minute); over it, 429 with `Retry-After`.

### AI investigation

`GET …/ai/status` returns `{ available, kind, label }` (`kind` is `rules`, `model` or `none`). `POST …/incidents/:id/investigations` with an optional `{ question }` (500 characters at most) queues an investigation and answers **202** with `status: "QUEUED"`; a worker fills it in. `GET` on the same path returns the five most recent, newest first. Each has `status` (`QUEUED`, `RUNNING`, `SUCCEEDED`, `FAILED`), `providerLabel`, `output` (summary, possible causes, evidence, recommended investigations and actions, confidence; present only when it succeeded and already verified), `sources` (the exact text the analysis read, each with a label such as `DEP-2`), counts of what verification removed (`droppedCitations`, `droppedClaims`, `downgradedCauses`), `truncated` and a short `error` when it failed. Reading needs `incidents.read`; starting needs `incidents.update`. Errors: 404 (unknown, malformed or foreign id), 409 `INVESTIGATION_ACTIVE` (one at a time per incident) or `AI_DISABLED`, 429 (10 per person per hour), 503 `QUEUE_UNAVAILABLE`.

### Real-time stream

`GET /orgs/:orgId/events` is a Server-Sent Events stream (`text/event-stream`) for members of the organisation (any role). It sends `event: ready` when connected, then `event: change` with `data: {"topic":"incidents"}` whenever something of that kind changed, plus a `: keep-alive` comment every `REALTIME_HEARTBEAT_MS` (default 15 s). Topics: `incidents`, `projects`, `services`, `monitoring`, `deployments`, `integrations`, `automation`, `notifications`, `members`, `organization`. A message never contains record data: refetch the matching REST resource. A member only receives topics they could read (`automation` and `integrations` reach administrators only), and `notifications` reaches only the person concerned. The stream ends when the member is removed, the user is disabled or the session ends. Errors: 401 (no session), 404 (not a member), 429 (more than 10 open streams for the user). Reverse proxies must not buffer the response (the API sends `X-Accel-Buffering: no`) and should allow idle connections for longer than the heartbeat.
...
Every change writes an `IncidentEvent` in the same transaction as the change. Event types so far: `CREATED`, `UPDATED`, `STATUS_CHANGED`, `SEVERITY_CHANGED`, `ASSIGNED`, `UNASSIGNED`, `COMMENT_ADDED`, `MONITORING_SIGNAL`, `DEPLOYMENT_LINKED`, `AUTOMATION_EXECUTED`. AI events are added by the phase that creates them.
...
## Phase 6 routes (automation, notifications, audit)

All under `/api/v1/orgs/:orgId`. The trigger catalogue, condition operators, action shapes and rule templates are defined once in `@nexus/shared`; the web app reads them from there rather than from an endpoint.

| Method and path                              | Permission            | Notes                                                                                                                                                                                                                                                                                                                                                                                                                              |
| -------------------------------------------- | --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /automation/rules`                      | `automation.manage`   | Rules, each with the outcome of its latest run.                                                                                                                                                                                                                                                                                                                                                                                    |
| `POST /automation/rules`                     | `automation.manage`   | Body `{ name, trigger, conditions?, actions, enabled?, cooldownSeconds? }`. Validated against the trigger: 400 with field-level `details` for an unknown field, wrong operator shape, a placeholder the trigger does not provide, or a recipient kind that does not fit. Unknown keys are dropped. Webhook destinations and named people must belong to this organization (400 otherwise). 409 `RULE_LIMIT` (50 per organization). |
| `GET/PUT/PATCH/DELETE /automation/rules/:id` | `automation.manage`   | `PUT` replaces the definition. `PATCH { enabled }` turns it on or off (a no-op writes no audit entry). `DELETE` is 204 and removes its run history; the audit entry is the permanent record. 404 for another organization's rule.                                                                                                                                                                                                  |
| `GET /automation/rules/:id/executions`       | `automation.manage`   | Newest first. `?limit=` (1-100) and `?before=<ISO timestamp>`; response has `nextBefore`. Each entry has its status, skip reason and per-action results (never a secret, address or payload).                                                                                                                                                                                                                                      |
| `GET /outbound-webhooks`                     | `automation.manage`   | Enabled destinations. URLs are shown as origin and path only.                                                                                                                                                                                                                                                                                                                                                                      |
| `POST /outbound-webhooks`                    | `integrations.manage` | Body `{ name, url }`. **201** with `signingSecret`: the only time it is ever returned. 400 `URL_NOT_ALLOWED` (same SSRF rules as monitoring), 409 `WEBHOOK_LIMIT` (10), 503 `INTEGRATIONS_NOT_CONFIGURED`.                                                                                                                                                                                                                         |
| `DELETE /outbound-webhooks/:id`              | `integrations.manage` | 204. Disables the destination; rules that still use it fail clearly rather than send.                                                                                                                                                                                                                                                                                                                                              |
| `GET /notifications`                         | `organization.read`   | **Your own** in-app notifications, newest first, with `unreadCount`. `?unread=true`, `?limit=`, `?before=`. The user always comes from the session, never a parameter.                                                                                                                                                                                                                                                             |
| `GET /notifications/unread-count`            | `organization.read`   | `{ count }`, for the bell.                                                                                                                                                                                                                                                                                                                                                                                                         |
| `POST /notifications/:id/read`               | `organization.read`   | 204, idempotent. 404 for a notification that is not yours.                                                                                                                                                                                                                                                                                                                                                                         |
| `POST /notifications/read-all`               | `organization.read`   | `{ updated }`. Only your own.                                                                                                                                                                                                                                                                                                                                                                                                      |
| `GET /audit-logs`                            | `audit.read`          | Newest first, `?action=` filter, `?limit=`, `?before=`. **Read-only**: there is no write route, and the table rejects UPDATE, DELETE and TRUNCATE.                                                                                                                                                                                                                                                                                 |

### Outbound webhook calls

A call is one JSON `POST`: `{ id, type, createdAt, organizationId, rule: { name }, data: <facts>, link }`, with the headers `X-Nexus-Event`, `X-Nexus-Delivery` (`<execution id>:<action index>`, constant across retries), `X-Nexus-Timestamp` (unix seconds) and `X-Nexus-Signature-256`. The signature is `sha256=` plus the hex HMAC-SHA256 of `"<timestamp>.<raw body>"` keyed with the signing secret. A receiver should reject a stale timestamp and deduplicate on the delivery id. A 2xx is success; 408, 425, 429 and 5xx are retried; other statuses are not. Redirects are not followed.
